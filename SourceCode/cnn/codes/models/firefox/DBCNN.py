...
pos = pd.read_csv('../../../../data/firefox/pos.csv')
neg = pd.read_csv('../../../../data/firefox/neg.csv')
sim = pd.read_csv('../../../../data/firefox/firefox_sim.csv')
sim = sim[['sim', 'label', 'pair_id']]
result_path = 'E:/Documents/project/pythonProject6/cnn/codes/models/firefox/'
...
# print(sim.shape)
# print(total.head())
#'same_tw', 'same_prio', 'same_comp',
sim = pd.merge(sim, total, on='pair_id', how='left')
sim = sim[['sim', 'label', 'pair_id', 'same_comp', 'same_tw', 'Title_1', 'Title_2', 'Issue_id_1', 'Issue_id_2']]
# print(sim.shape)
#sim = sim[['sim', 'label', 'pair_id', 'same_comp', 'Title_1', 'Title_2', 'Issue_id_1', 'Issue_id_2']]
# print(sim.head())
train_set = sim.head(int(0.8*len(sim)))[['sim']].fillna(0)

#train_set = sim.head(int(0.8*len(sim)))[['sim', 'same_comp']].fillna(0)
train_label = sim.head(int(0.8*len(sim)))['label'].apply(lambda x: int(x))

test_set = sim.tail(int(0.2*len(sim)))[['sim']].fillna(0)
#test_set = sim.tail(int(0.2*len(sim)))[['sim', 'same_comp']].fillna(0)
...
f = open(result_path + 'DBCNN.txt', 'a')
sys.stdout = f
print('precision'+' '+'{:.6f}' .format(precision))
print('recall'+' '+'{:.6f}' .format(recall))
print('accuracy'+' '+'{:.6f}' .format(float(cnn_cnt)/len(res)))
print('f1'+' '+'{:.6f}' .format(f1))
print('TP' + ' ' + '%f' % t)
print('FP' + ' ' + '%f' % ((cnt - t)))
print('TN' + ' ' + '%f' % (cnn_cnt - t))
print('FN' + ' ' + '%f' % (sum(list(validation_label)) - t))
